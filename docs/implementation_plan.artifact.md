...
- Paket adı `com.example.test_app` → `io.github.yildizib.mirrorline` olacak.
- Uygulama başlığı/adı `test_app` → `MirrorLine` olacak.

## Open Questions
- Yeni paket adı `io.github.yildizib.mirrorline` uygun mu? (Think & Solve Different Solutions FZE bağlamında)
...
- `namespace` ve `applicationId` → `io.github.yildizib.mirrorline`.

#### [MODIFY] android/app/src/main/AndroidManifest.xml
- `android:label` → `MirrorLine`.

#### [MODIFY] android/app/src/main/kotlin/com/example/test_app/MainActivity.kt
- Paket yolu ve dosya konumu → `io.github.yildizib.mirrorline`.
