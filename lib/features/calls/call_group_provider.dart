...
/// All calls from the same number (or same "Bilinmeyen numara" placeholder
...
class CallGroup {
  /// The grouping key: contact name if known, else the raw number, else the
  /// placeholder. All calls in this group share this display identity.
  final String key;
  ...
  /// ringing, show "Çalıyor"; otherwise the most recent call's status.
  String get statusLabel {
    if (hasActive) return 'Çalıyor';
    return lastCall.statusLabel;
  }
}

/// Derives call groups from callListProvider's flat list, grouped by
/// display identity (contact name -> number -> placeholder). Sorted with
/// the most recently active group first.
