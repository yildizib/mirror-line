...
void main() {
  ...
  test('beginForceConnect clears log and sets active flag', () {
    ...
    expect(notifier.state.discoveryLog, isEmpty, reason: 'new session starts clean');
  ...
  test('endForceConnect clears active flag and detail but keeps state', () {
    final notifier = ConnectionStatusNotifier();
    notifier.beginForceConnect();
    notifier.setDiscoveryState(DiscoveryState.connecting, detail: 'Connecting to 1.2.3.4');
  ...
  test('setDiscoveryState updates state and detail', () {
    final notifier = ConnectionStatusNotifier();
    notifier.setDiscoveryState(DiscoveryState.scanningSubnet, detail: 'Scanning batch 3/11');
  ...
  test('recordConnectAttempt increments count and sets error', () {
    final notifier = ConnectionStatusNotifier();
    notifier.recordConnectAttempt(ConnectionErrorCode.connectFailed, errorDetail: '1.2.3.4:45678');
  ...
  test('discovery state transitions match a typical force-connect flow', () {
    ...
    // Trying stored IP.
    notifier.setDiscoveryState(DiscoveryState.tryingStoredIp, detail: 'Trying 1.2.3.4...');
    notifier.logDiscovery('Trying stored IP 1.2.3.4...');
    expect(notifier.state.discoveryState, DiscoveryState.tryingStoredIp);

    // Fails -> scanning.
    notifier.setDiscoveryState(DiscoveryState.scanningSubnet, detail: 'Scanning 192.168.1.0/24 (batch 1/11)');
    notifier.logDiscovery('Stored IP failed', isError: true);
    notifier.logDiscovery('Scanning subnet...');
    expect(notifier.state.discoveryState, DiscoveryState.scanningSubnet);

    // Scan finds host -> connecting.
    notifier.setDiscoveryState(DiscoveryState.connecting, detail: 'Connecting to 192.168.1.99...');
    ...
    // Connected.
    notifier.setDiscoveryState(DiscoveryState.connected, detail: 'Connected to 192.168.1.99');
    notifier.logDiscovery('Connected!', isSuccess: true);
    notifier.endForceConnect();
