...
void main() {
  ...
  test(
    'pairingRequest carries scanner ip/role/deviceName and is encrypted',
    () async {
      ...
      final payload = await receivedRequest.future.timeout(
      ...
      expect(
        ...
        '192.168.1.42',
        reason:
            'scanner must claim its live local IP so the scanned side '
  ...
  test(
    'pairingAccept carries scanned ip/role/deviceName and is encrypted',
    () async {
      final key = CryptoManager.generateKey();
      ...
      final payload = await receivedAccept.future.timeout(
        const Duration(seconds: 5),
      );
      ...
      expect(
        ...
        'source',
        reason:
            'scanned device must claim its role so the scanner can '
      ...
      expect(
        ...
        '192.168.1.99',
        reason:
            'scanned device must claim its live local IP so the scanner '
            'can store it as the peer IP',
      );
  ...
  test(
    'pairingAccept IP overrides the QR IP when persisting the peer',
    () async {
      ...
      expect(
        ...
        acceptIp,
        reason:
            'scanner must prefer the live IP from pairingAccept over '
